...
/**
 * `migrations/` sits at the package root, beside `src/` and `dist/`, so resolve up out of
 * `core/data/` in whichever of the two we are running from.
 */
 ...
 * filename order, each in its own transaction. Safe to run on every process start, by whichever of them starts first.
 */
