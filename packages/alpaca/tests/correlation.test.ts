...
describe('Alpaca order correlation', () => {
  it('round-trips every field', () => {
    const correlation = { virtualAccountId: 'MOMENTUM01', reservationId: 'RES1', groupId: 'group-abc' };
  ...
  it('round-trips a partial correlation without inventing the missing fields', () => {
    ...
    expect(decodeAlpacaOrderCorrelation(encoded)).toEqual({ virtualAccountId: 'MOMENTUM01', reservationId: undefined, groupId: undefined });
  });

  it('encodes in the form Alpaca will echo back', () => {
    expect(encodeAlpacaOrderCorrelation({ virtualAccountId: 'ACC1', groupId: 'G1' })).toBe('_c@a:ACC1;g:G1');
  });
  ...
  it('rejects an id containing a separator rather than producing one that misparses', () => {
    ...
    expect(() => encodeAlpacaOrderCorrelation({ groupId: 'g:1' })).toThrow(InvalidRequestError);
  });
  ...
  it('ignores segments it does not recognise', () => {
    expect(decodeAlpacaOrderCorrelation('_c@a:ACC1;x:whatever;g:G1')).toEqual({ virtualAccountId: 'ACC1', reservationId: undefined, groupId: 'G1' });
  });
});
